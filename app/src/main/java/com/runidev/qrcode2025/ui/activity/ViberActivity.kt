...
class ViberActivity : BaseActivity<ActivityViberBinding>(ActivityViberBinding::inflate) {
    ...
    private fun initHandle() {
        binding.apply {
            val countryCodeWithPlus = phonePicker.selectedCountryCodeWithPlus
            val countryCode = phonePicker.selectedCountryCode
            phonePicker.setOnCountryChangeListener {
                val code = phonePicker.selectedCountryCodeWithPlus
                codeChoose = code
            }
            tabCreate.clicks {
                ...
                if (dataClipBoard) {
                    ...
                    if (checkExistData == 0) {
                        val clipboardModel = QrCode(
                            ...
                            codeChoose + "-${editNumberPhone.text}",
                            false,
                            R.drawable.viber,
                            false
                        )
